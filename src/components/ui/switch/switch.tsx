...
import { SwitchColorScheme, SwitchProps, SwitchSize } from './switch.types';
...
const Switch = memo(
  ...
  >(({ className, size = 'default', colorScheme = 'default', ...props }, ref) => {
    // Generate the root class names with appropriate size and color scheme modifiers
    const rootClassName = useMemo(() => {
      return cn(styles['switch-root'], className);
    }, [className]);
    ...
    return (
      <SwitchPrimitives.Root className={rootClassName} ref={ref} {...props}>
