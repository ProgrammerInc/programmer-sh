...
export const AlertDialogAction = memo(
  React.forwardRef<React.ElementRef<typeof AlertDialogPrimitive.Action>, AlertDialogActionProps>(
    ({ className, ...props }, ref) => {
      const actionClassName = useMemo(() => {
        return cn(buttonVariants(), className);
      }, [className]);

      return <AlertDialogPrimitive.Action ref={ref} className={actionClassName} {...props} />;
    }
  )
);
