...
import { loadCommandHistory, scrollToBottom } from './terminal.utils';
 ...
 */
const Terminal = memo(
  forwardRef<HTMLDivElement, TerminalProps>(
    (
      {
        initialCommands = [],
        className = '',
      ...
      const { isProcessingAsync, processCommand, addCommandToQueue } = useCommandProcessor(
        initialCommands,
        setCommandOutput,
      ...
      // Ensure welcome command runs on first load (after clear command)
      useEffect(() => {
        // Only run if processCommand is available
        if (processCommand) {
          // Check if welcome or clear are already in initialCommands
          const hasWelcomeCommand = initialCommands.some(cmd => cmd === 'welcome');
          const hasClearCommand = initialCommands.some(cmd => cmd === 'clear');

          // Run clear and welcome commands in sequence if they're not already in initialCommands
          if (!hasWelcomeCommand && !hasClearCommand && commandOutput.length === 0) {
            // First clear the terminal, then show welcome
            processCommand('clear')
              .then(() => processCommand('welcome'))
              .catch(err => console.error('Error processing startup commands:', err));
          }
        }
      }, [processCommand, initialCommands, commandOutput.length]);

      // Process initial commands
      useEffect(() => {
        if (initialCommands.length > 0) {
          initialCommands.forEach(cmd => {
            processCommand(cmd).catch(err =>
              console.error('Error processing initial command:', err)
            );
          });
        }
      ...
      const handleCommandSubmit = useCallback(
        async (e: FormEvent<HTMLFormElement> | string) => {
          let commandString = '';
      ...
      const themeClass =
        resolvedTheme === 'dark' ? styles['terminal-dark'] : styles['terminal-light'];
      ...
      return (
        ...
        >
          ...
          <div ref={terminalContentRef} className={styles['terminal-content']}>
            <TerminalContent
