...
import { CSS_CLASSES, SVG } from './sphere-animation.constants';
import { useSphereAnimation } from './sphere-animation.hooks';
import styles from './sphere-animation.module.css';
import { SphereAnimationProps } from './sphere-animation.types';
import { cn } from './sphere-animation.utils';

/**
 * Sphere Animation Component
 *
 * Renders an interactive animated sphere with configurable color schemes.
 * The animation includes breathing effects, shadow movement, and gradient transitions.
 *
 * @param {SphereAnimationProps} props - Component properties
 * @returns {JSX.Element} The rendered component
 */
export const SphereAnimation: FC<SphereAnimationProps> = memo(function SphereAnimation({
  className,
  colorScheme = 'default'
}: SphereAnimationProps) {
  const sphereRef = useRef<SVGSVGElement>(null);

  // Initialize animations with the sphere reference and color scheme
  useSphereAnimation(sphereRef, colorScheme);

  return (
    <div className={cn(styles[CSS_CLASSES.CONTAINER], className)}>
      <div className={styles[CSS_CLASSES.WRAPPER]}>
        <div className={styles[CSS_CLASSES.CANVAS]}>
          <svg
            ref={sphereRef}
            className="sphere"
            viewBox={SVG.VIEWBOX}
            stroke={SVG.DEFAULT_STROKE}
            aria-hidden="true"
          >
            <defs>
              <linearGradient
            ...
            <path
              className={`${CSS_CLASSES.PATH} ${SVG.GRADIENT_ID}-path`}
              fill={`url(#${SVG.GRADIENT_ID})`}
