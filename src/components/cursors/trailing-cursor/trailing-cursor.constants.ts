/**
 * TrailingCursor Constants Module
 *
 * Constants used by the TrailingCursor component, including default values
 * for particles, animation rate, and accessibility preferences.
 *
 * @module TrailingCursorConstants
 */

/**
 * Default base image for particles in base64 format
 * This is a small drop-like shape with a slight tail
 */
export const DEFAULT_BASE_IMAGE_SRC =
  // eslint-disable-next-line no-secrets/no-secrets
  'data:image/png;base64,';

/**
 * Default number of particles in the trail
 */
export const DEFAULT_PARTICLES_COUNT = 15;

/**
 * Default rate at which particles follow the cursor
 */
export const DEFAULT_RATE = 0.4;

/**
 * Default z-index for the canvas element
 */
export const DEFAULT_Z_INDEX = '99999';

/**
 * Default style settings for the canvas
 */
export const DEFAULT_CANVAS_STYLES = {
  position: 'fixed' as const,
  top: '0px',
  left: '0px',
  pointerEvents: 'none' as const,
  zIndex: DEFAULT_Z_INDEX,
  transform: 'translateZ(0)' // Create stacking context with hardware acceleration
};

/**
 * Media query for reduced motion preference
 */
export const REDUCED_MOTION_QUERY = '(prefers-reduced-motion: reduce)';
