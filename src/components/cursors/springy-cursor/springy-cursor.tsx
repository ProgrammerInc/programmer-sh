...
export const SpringyCursor: React.FC<SpringyCursorProps> = ({ emoji = '🍆', wrapperElement }) => {
      ...
      for (let i = 1; i < nDots; i++) {
        ...
        if (particlesRef.current[i].position.y >= height - DOTSIZE - 1) {
          if (particlesRef.current[i].velocity.y > 0) {
            particlesRef.current[i].velocity.y = BOUNCE * -particlesRef.current[i].velocity.y;
          }
        ...
        if (particlesRef.current[i].position.x >= width - DOTSIZE) {
          if (particlesRef.current[i].velocity.x > 0) {
            particlesRef.current[i].velocity.x = BOUNCE * -particlesRef.current[i].velocity.x;
          }
