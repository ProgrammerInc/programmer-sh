...
export function processCommandLinks(text: string, onCommandClick?: (command: string) => void): LinkMatch[] {
  const matches: LinkMatch[] = [];
...
export function processLinks(text: string, onCommandClick?: (command: string) => void): React.ReactNode[] {
  if (!text) return [text];
  ...
  // Combine all matches
  const allMatches = [...urlMatches, ...emailMatches, ...commandMatches, ...phoneMatches]
