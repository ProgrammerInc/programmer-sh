...
export const useCommandExecution = (commands: Record<string, Command>) => {
  ...
  const executeCommand = useCallback(
    (commandStr: string) => {
      ...
      if (
        cleanCommand.startsWith('__event_') ||
        cleanCommand.startsWith('__init_') ||
        cleanCommand.startsWith('__url_')
      ) {
      ...
      if (!command) {
        console.warn(
          `Command not found: '${commandName}', available commands: ${Object.keys(commands).join(', ')}`
        );
