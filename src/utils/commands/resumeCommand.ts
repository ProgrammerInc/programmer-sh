  ...
  execute: () => {
    return {
      ...
      asyncResolver: async (): Promise<CommandResult> => {
        ...
        const github = portfolioData.contact.github && !portfolioData.contact.github.startsWith('http') 
          ? `https://${portfolioData.contact.github}` 
          : portfolioData.contact.github;
        
        const linkedin = portfolioData.contact.linkedin && !portfolioData.contact.linkedin.startsWith('http') 
          ? `https://${portfolioData.contact.linkedin}` 
          : portfolioData.contact.linkedin;
        
        const twitter = portfolioData.contact.twitter && !portfolioData.contact.twitter.startsWith('http') 
          ? `https://${portfolioData.contact.twitter}` 
          : portfolioData.contact.twitter;
        
        const website = portfolioData.contact.website && !portfolioData.contact.website.startsWith('http') 
...
<div class="resume-section">
  ...
  ${edu.details ? `<span class="details">${edu.details}</span>` : ''}
  </div>`
    )
    .join('')}
</div>
  ...
  <div class="skills-section">
    ${portfolioData.skills.map(skillCategory => 
      `<div class="skill-category">
        <span class="category"><strong>${skillCategory.category}:</strong> ${skillCategory.items.join(', ')}</span>
      </div>`).join('')}
  </div>
</div>
...
<div class="resume-section">
  ...
  <div class="contact-section">
    ...
    ${website ? `<span><strong>Website:</strong> <a href="${website}" target="_blank" class="text-terminal-link hover:underline">${website.replace(/^https?:\/\//, '')}</a></span>` : ''}
  </div>
</div>`,
          isError: false,
        };
